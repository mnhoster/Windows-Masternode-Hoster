...
namespace MasternodeHoster
{
  public class BitnetClient : IBitnetClient
  {
    ...
    public JObject MNSync()
    {
      return InvokeMethod("mnsync status")["result"] as JObject;
    }

        public string GetMasternodeOutputs()
        {
            return InvokeMethod("getmasternodeoutputs")["result"].ToString();
        }
        public JArray GetMasternodeStatus()
        {
            return InvokeMethod("getmasternodestatus")["result"] as JArray;
        }
